...
Determine the _quality level_ of each blueprint by _multiplying that blueprint's ID number_ with the largest number of geodes that can be opened in 24 minutes using that blueprint. In this example, the first blueprint has ID 1 and can open 9 geodes, so its quality level is `9`. The second blueprint has ID 2 and can open 12 geodes, so its quality level is `24`. Finally, if you _add up the quality levels_ of all of the blueprints in the list, you get `33`.
